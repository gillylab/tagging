...
elseif TagLocation == 0 % GOC
    switch tagNumD
        ...
        case 60970
            tagID = '2';
        case 64006 
            a = datevec(Seriesdate(1),1);
            if a(1) == 2006 % 2006 tag
                tagID = '3';
            elseif a(1) == 2008 % 2008 tag
                tagID = '7';
            end
        case 62007
            tagID = '4';
    ...
    buffernum = 1; % set July 27
    buffer = datenum(0000,00,00,buffernum,00,00); %within xx of sunset
    iAscent = (SeriesdateVtimes>=0) & (SeriesdateVtimes<(sset+buffer)); % sunset is right about 01:00, so just do it the hour before to keep it on the same day.
    iDescent = (SeriesdateVtimes>=(srise-buffer)) & (SeriesdateVtimes<(srise+buffer));
    ...
    Boxplotrix = [repmat(tagNumD,length(DepthD),1) DEPLOYINDEXN DepthN DepthD TempN TempD DEPLOYINDEX-DeployDays(1)+1]; % this will be saved as a .mat file
    
    %% Make histogram bins
    
    % depth bins
   % binsizeD = 25; % for thesis
    binsizeD = 50; % for NSFreport
    ...
    % for MBARI comparison. must change binsizeD = 50;
%     dd = dbar;
%     save([CompareTagDir tagName '_DepthHistoDaily.mat'], 'DepthNperc_daily', 'DepthDperc_daily', 'dd')
